"""
    N-body simulation.(Reduce Function Call Overhead)
"""
...
# Modified(Reduce function call)
def aggregate_advance(dt, body1, body2, seenit):
    if (body1 != body2) and not (body2 in seenit):
        ([x1, y1, z1], v1, m1) = BODIES[body1]
        ([x2, y2, z2], v2, m2) = BODIES[body2]
...
def advance(dt):
    ...
    for body1 in BODIES.keys():
        for body2 in BODIES.keys():
            aggregate_advance(dt, body1, body2, seenit)
...
# Modified
def aggregate_report_energy(body1, body2, seenit):
    if (body1 != body2) and not (body2 in seenit):
        ((x1, y1, z1), v1, m1) = BODIES[body1]
        ((x2, y2, z2), v2, m2) = BODIES[body2]
        (dx, dy, dz) = (x1-x2, y1-y2, z1-z2)
...
def report_energy(e=0.0):
    ...
    for body1 in BODIES.keys():
        for body2 in BODIES.keys():
            e -= aggregate_report_energy(body1, body2, seenit)
